...
class BoundingBoxView {
  ...
  /// - Parameters:
  ///   - frame: The CGRect frame defining the bounding box's size and position (Vision coordinates 0-1).
  ...
  ///   - bounds: The bounds of the parent view for coordinate conversion.
  func show(frame: CGRect, label: String, color: UIColor, alpha: CGFloat, bounds: CGRect) {
    CATransaction.setDisableActions(true)  // Disable implicit animations

    // Convert Vision coordinates (0-1, origin bottom-left) to view coordinates (origin top-left)
    let convertedFrame = CGRect(
      x: frame.minX * bounds.width,
      y: (1 - frame.maxY) * bounds.height,  // Flip Y coordinate
      width: frame.width * bounds.width,
      height: frame.height * bounds.height
    )

    let path = UIBezierPath(roundedRect: convertedFrame, cornerRadius: 6.0)  // Rounded rectangle for the bounding box
    ...
    let textOrigin = CGPoint(x: convertedFrame.origin.x - 2, y: convertedFrame.origin.y - textSize.height - 2)  // Position above the bounding box
